...
1. **Push this folder to a GitHub repo.** The repo can be public or private.

2. **Confirm `vite.config.js`** has `base: "/Cairn/"` if your repo is named `Cairn`. If your repo is named differently, change the base path to `"/<your-repo-name>/"` (case-sensitive).

3. **Enable GitHub Pages:**
   - Go to **Settings → Pages**
   - Under **Source**, select **GitHub Actions**

4. **Push to `main`.** The workflow in `.github/workflows/deploy.yml` runs automatically: install dependencies, build, publish to GitHub Pages.

5. **Wait ~2 minutes**, then visit `https://<your-username>.github.io/Cairn/`.
...
Copy any seeded state's structure, swap in your own data, and the engine handles the rest.
...
This prototype provides preliminary eligibility estimates only and is not a guarantee of benefits. Income and asset thresholds change annually. Always confirm details with the listed agencies before making decisions. Not affiliated with or endorsed by Medicare or any government agency.
